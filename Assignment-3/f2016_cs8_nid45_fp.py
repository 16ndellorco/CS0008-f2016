...
# the furthest, the least, and shows how many people show up in multiple .csv files. Also an added
# class called "participant".


class Participant:
    def __init__(self, name, distance=0, runs=0):
        #set the name
        self._name = name
        #Set distance and runs accordingly
    ...
    def addDistances(self, ld):
        for i in ld:
            self._distance += i
#return the number of distances computed
...
#stringify the object
    def __str__(self):
        return 'Name : {:20}. Distance run : {:09.2f}. Runs : {:04d} \n'.format(self._name, self._distance, self._runs)
...
def get_stats(info):
    ...
    for key in info.keys():
        ...
        for item in info[key]:
            ...
            if item[0].strip() not in participants.keys():
                d = Participant(item[0].strip(), distance, 1)
                participants[item[0].strip()] = d
            else:
                participants[item[0].strip()].addDistance(distance)
    ...
    for participant in participants.keys():
        if participants[participant]._runs > 1:
    ...
    with open('f2016_cs8_nid45_a3.data.output.csv', 'w') as filehandle:
        for key in participants.keys():
            filehandle.write(str(participants[key]))
...
def main():
    ...
    masterfile = input(
        "What is the name of the materfile? ")  # ask for masterfile name to be inputted in case it was changed
    if masterfile.lower() not in ['q', 'quit', '']:
